...
if [ ! -f "./IpAndPort" -a  ! -s "./IpAndPort"  ] ; then
    ...
    if [ $setflag = "y" ];then	
	    sum=$ipnum+$portnum
		for (( ip=1,port=$sum; ip<$ipnum,port>$ipnum;ip++,port--))
		do 
			echo "IP$ip"
			eval IP$ip=$(cat ./IpAndPort |awk -F" " -v nip=$ip '{print $nip}')
		    echo "Port$[$sum-$port+1]"
			eval Port$[$sum-$port+1]=$(cat ./IpAndPort |awk -F" " -v nport=$port '{print $nport}')
        done				
    else
...
for ((port=1; port<=$portnum;port++))
do
    redis-server /etc/RedisCluster/$[Port$port]/redis.conf
done

if [ ! -a "/etc/init.d/redis_cluster" ]; then
    for ((port=1; port<=$portnum;port++))
    do
	    echo "redis-server /etc/RedisCluster/$[Port$port]/redis.conf" > /etc/init.d/redis_cluster
    done
